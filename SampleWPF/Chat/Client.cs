...
namespace ChatUI.Chat
{
    public class Client : IClient
    {
        ...
        public void Start()
        {
            ...
            listenThread = new Thread(this.Listen); //Crear Hilo
            listenThread.SetApartmentState(ApartmentState.STA); //Se establece para actualizar la interfaz grafica por medio de otro hilo que no sea el principal
            listenThread.Start();
        ...
        {
            if (user.id != this.u.id && this.chat.AddUser(user))  //Comprobar si somos nosotros el usuario o ya tenemos ese usuario agregado
            {                 
                onUpdate(user);                
            }
        ...
        {
            this.chat.AddMessage(m); //Agregar mensaje a la "db"
            onUpdate(m);//Actualizar la interfaz grafica
        }
